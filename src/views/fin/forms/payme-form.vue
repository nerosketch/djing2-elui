...
import { IPayPaymeGateway } from '@/api/fin/types'
import { addPaymePayGateway, changePaymePayGateway, getPaymePayGateway } from '@/api/fin/req'
...
export default class extends Vue {
  ...
  private changeGw(dat: IPayPaymeGateway) {
  ...
  private async fetchGw() {
    if (!this.isNew) {
      const { data } = await getPaymePayGateway(this.$store.state.basepaymentmodel.id)
      this.changeGw(data)
    }
  }
  ...
  private onSubmit() {
    (this.$refs.form as Form).validate(async valid => {
      if (valid) {
        ...
        try {
          if (this.isNew) {
            newDat = await addPaymePayGateway(this.frmMod)
          } else {
            newDat = await changePaymePayGateway(
